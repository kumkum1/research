...
from data import WORD_LEN, text_df

# text_df = pd.read_csv('data/processed/text_data.csv') 

all_words = ["_skip"] * WORD_LEN 
...
all_words = [w for w in all_words if w != "_skip"]
...
for i in range(len(all_words) - window_size - 1):
    ...
    window_words = [w for w in window] # excluding SKIP placeholders
...
# counts = recall_df.groupby('word')['dummy'].agg('sum')
# word_stats['count'] = counts
